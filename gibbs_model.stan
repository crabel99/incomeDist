...
functions {
  real gibbs_dist(real x,         // Function argument
  ...
  real gibbs_int( real[] limits, real[] theta, data real[] x_r) {
    int x_i[0];
...
transformed parameters {
  ...
  for (n in 1:N)
    if (n < N)
      y_hat[n] = gibbs_int({x[n], x[n + 1]}, {T, r0, alpha}, x_r);
    else
      y_hat[n] = gibbs_int({x[n], positive_infinity()}, {T, r0, alpha}, x_r);
